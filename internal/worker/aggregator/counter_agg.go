...
func (f *counter) Process(delivery amqp.Delivery, headers amqp.Header) ([]sequence.Destination, []byte) {
...
func (f *counter) processEof(headers amqp.Header, recovery bool) []sequence.Destination {
	var sequenceIds []sequence.Destination
...
func (f *counter) publish(headers amqp.Header, eof bool) []sequence.Destination {
	var sequenceIds []sequence.Destination
...
func (f *counter) sendBatch(headers amqp.Header, b []byte) []sequence.Destination {
...
// TODO extract
func (f *counter) sendEof(headers amqp.Header) []sequence.Destination {
	output := shardOutput(f.w.Outputs[0], headers.ClientId)
...
func (f *counter) reset(clientId string) {
	delete(f.eofsRecv, clientId)
	delete(f.games, clientId)
}

func (f *counter) saveGame(msgBytes []byte, clientId string) {
...
func (f *counter) batchSizeReached(headers amqp.Header) bool {
	return len(f.games[headers.ClientId]) >= int(f.batchSize)
}

func (f *counter) eofsReached(headers amqp.Header) bool {
