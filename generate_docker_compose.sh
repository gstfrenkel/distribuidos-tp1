...
read -p "Enter the number of percentile filter instances: " percentile_count
...
python3 generate_docker_compose.py "$reviews_count" "$review_text_count" "$action_count" "$indie_count" "$platform_count" "$joiner_counter_count" "$joiner_top_count" "$joiner_percentile_count" "$percentile_count" "$topn_count" "$platform_counter_count" "$topn_playtime_count" "$release_date_count"
